...
Έχουν προστεθεί περισσότερες εντολές για μενού, καρτέλες, διαλόγους, λίστες μεταβλητών, πίνακες, Output Viewer, πίνακες αποτελεσμάτων, Variable View, ελληνικό γλωσσάρι και προαιρετικές αυτόματες ανακοινώσεις κελιών.

Για να γίνει ακόμη πιο πλήρες, απομένουν κυρίως:

- Πραγματικές δοκιμές με ελληνικό και αγγλικό SPSS σε SPSS 29, 30 και 31, ώστε να προστεθούν ακριβέστεροι κανόνες για διαφορετικά ονόματα παραθύρων και controls.
- Συλλογή ανώνυμων αναφορών από την εντολή `NVDA+Control+Alt+Shift+G`, ώστε να καλυφθούν μη επισημασμένα κουμπιά και dialogs που δεν εμφανίζονται σε όλες τις εγκαταστάσεις.
- Καλύτερη ανάγνωση του Workbook mode, επειδή το SPSS μπορεί να εμφανίζει output και syntax σε workbook αντί για classic windows.
- Πιο πλούσια προαιρετική λειτουργία εκμάθησης για νέους χρήστες SPSS στα ελληνικά, με οδηγίες ανά εργασία και όχι μόνο ανά παράθυρο.
- Οδηγός για προσβάσιμη εργασία με syntax και OMS, ώστε μη προσβάσιμα charts ή model output να αποθηκεύονται ως κείμενο ή δεδομένα.
- Αυτοματοποιημένο πακέτο δοκιμών για τους κανόνες ανίχνευσης panes, labels και table context με ψεύτικα NVDA objects.
- Δημοσίευση στο NVDA Add-on Store όταν ολοκληρωθεί αρκετή δοκιμή σε πραγματικά Windows/SPSS περιβάλλοντα.
...
- `addon/`: μόνο τα αρχεία που ανήκουν στο πακέτο NVDA add-on.
- `dist/`: τοπικά παραγόμενα αρχεία `.nvda-addon` και SHA256 checksums.
- `docs/`: changelog στα ελληνικά, checklist έκδοσης και τεχνικές σημειώσεις.
- `store/`: στοιχεία που χρειάζονται για υποβολή στο NVDA Add-on Store.
- `.github/workflows/`: αυτοματοποιημένος έλεγχος build και πακέτου στο GitHub.

## Λίστα Ελέγχου Δοκιμών

1. Ξεκινήστε το NVDA.
2. Εγκαταστήστε το πρόσθετο και επανεκκινήστε το NVDA.
3. Ανοίξτε το IBM SPSS Statistics.
4. Ανοίξτε ή δημιουργήστε dataset.
5. Πατήστε `NVDA+Control+Alt+U`, `D` και `V` στον Data Editor και ελέγξτε ανακοινώσεις Overview, Data View και Variable View.
6. Μετακινηθείτε σε κελιά με τα βελάκια και πατήστε `NVDA+Control+Alt+Shift+D`.
7. Μετακινηθείτε σε ιδιότητες μεταβλητών και πατήστε `NVDA+Control+Alt+Shift+V`, ειδικά στα Type, Values και Missing.
8. Πατήστε `NVDA+Control+Alt+Shift+R` σε γραμμή Variable View και ελέγξτε τη σύνοψη μεταβλητής.
9. Ενεργοποιήστε `NVDA+Control+Alt+Shift+A`, μετακινηθείτε με βελάκια σε πίνακα και ελέγξτε τις αυτόματες ανακοινώσεις.
10. Εκτελέστε ανάλυση, για παράδειγμα Frequencies ή Descriptives.
11. Μεταβείτε στο Output Viewer και πατήστε `NVDA+Control+Alt+Shift+O`, `U`, `Y` και `C`.
12. Πατήστε `NVDA+Control+Alt+Shift+E` και ελέγξτε την καθοδήγηση προσβασιμότητας αποτελεσμάτων.
13. Ανοίξτε Graphs, Chart Builder και πατήστε `NVDA+Control+Alt+C`.
14. Πατήστε `NVDA+Control+Alt+W`, `NVDA+Control+Alt+Shift+M`, `N`, `I`, `L` και `F` σε σχετικά σημεία του SPSS.
15. Πατήστε `NVDA+Control+Alt+Shift+B` και ελέγξτε ότι οι ανακοινώσεις γίνονται σύντομες.
16. Πατήστε `NVDA+Control+Alt+Shift+G` και ελέγξτε το log του NVDA για αναφορά αντικειμένου.
...
The add-on now includes expanded commands for menus, tabs, dialogs, variable lists, tables, Output Viewer, output tables, Variable View, a Greek glossary, and optional automatic table-cell announcements.

The remaining work is mainly:

- Real testing with Greek and English SPSS on SPSS 29, 30, and 31, so version-specific window and control names can be handled more accurately.
- A small anonymized corpus of reports from `NVDA+Control+Alt+Shift+G`, so unlabeled buttons and dialogs can be covered across more installations.
- Better Workbook mode support, because SPSS can display output and syntax in workbook mode instead of classic windows.
- A richer optional learning mode for new SPSS users in Greek, with task-by-task guidance instead of only pane-by-pane guidance.
- A guide for accessible syntax and OMS workflows, so inaccessible charts or model output can be captured as text or data.
- Automated tests for pane detection, labels, and table context using fake NVDA objects.
- Publication through the NVDA Add-on Store after enough real Windows/SPSS testing.
...
- `addon/`: files that belong inside the NVDA add-on package.
- `dist/`: locally generated `.nvda-addon` files and SHA256 checksums.
- `docs/`: Greek changelog, release checklist, and technical notes.
- `store/`: Add-on Store submission metadata and issue-form values.
- `.github/workflows/`: GitHub automation for validation and package checks.

## Testing Checklist

1. Start NVDA.
2. Install the add-on and restart NVDA.
3. Open IBM SPSS Statistics.
4. Open or create a dataset.
5. Press `NVDA+Control+Alt+U`, `D`, and `V` in the Data Editor and verify Overview, Data View, and Variable View announcements.
6. Move through cells with arrow keys and press `NVDA+Control+Alt+Shift+D`.
7. Move through variable properties and press `NVDA+Control+Alt+Shift+V`, especially on Type, Values, and Missing.
8. Press `NVDA+Control+Alt+Shift+R` on a Variable View row and verify the variable summary.
9. Turn on `NVDA+Control+Alt+Shift+A`, move with arrow keys in a table, and verify automatic cell announcements.
10. Run an analysis, for example Frequencies or Descriptives.
11. Move to the Output Viewer and press `NVDA+Control+Alt+Shift+O`, `U`, `Y`, and `C`.
12. Press `NVDA+Control+Alt+Shift+E` and verify output accessibility guidance.
13. Open Graphs, Chart Builder and press `NVDA+Control+Alt+C`.
14. Press `NVDA+Control+Alt+W`, `NVDA+Control+Alt+Shift+M`, `N`, `I`, `L`, and `F` in relevant SPSS areas.
15. Press `NVDA+Control+Alt+Shift+B` and verify shorter pane announcements.
16. Press `NVDA+Control+Alt+Shift+G` and check NVDA log for an object report.
